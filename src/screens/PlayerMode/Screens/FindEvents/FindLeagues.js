...
import PlayerToBeRequestedEventsCards from '../../Cards/PlayerToBeRequestedEvents';
  
class FindLeaguesScreen extends Component {
  constructor(props) {
    super(props);
    this.data=''
        this.state = {
  ...
  getData =()=>{
    ...
    var dummyData =[]
    // if(type==0){
    //   gettingUrl = 'https://pickletour.com/api/get/bothLeagues/page/'
    // }
    // else if(type==1){
    //   gettingUrl = 'https://pickletour.com/api/get/League/page/'
    // }
    // else{
    //   gettingUrl = 'https://pickletour.com/api/get/usapaLeague/page/'
    // }
  ...
  searchUSAPASantioned(){
   ...
   if(backupData.length>0){
    newData.map(item=>{
      if(item.type=='Sanctioned League'){
        dummyData.push(item)
      }
    })
   }
                ...
                {options.map(item=>{
                    return(<View key={item.key} style={styles.buttonContainer} >
                      <TouchableOpacity style={{flexDirection:'row'}} onPress={()=>this.sendCallForFilter(item.key)}>
                        ...
                        { value ==item.key ? 
                        <View style={{backgroundColor:'#15C9EB', paddingHorizontal:10,paddingVertical:5, borderRadius:5, borderWidth:1, borderColor:'#019EBC'}}>
                          <Text style={{fontFamily:'Lato-Medium', fontSize:Responsive.font(12), color:'white'}}>  {item.text}  </Text>
                        </View>
                        
                        :
                        <View style={{paddingHorizontal:10, paddingVertical:5, borderRadius:5, borderWidth:1, borderColor:'#019EBC'}}>
        ...
        <View style={{borderWidth:0.5,borderColor:'#E2E2E2', marginRight:10, marginLeft:10, marginBottom:10}}></View>
          <FlatList
            ...
            renderItem={({item})=>(
              ...
              <PlayerToBeRequestedEventsCards key={item._id} data={item} user={this.data} />
            </TouchableOpacity>
            )}
            ListFooterComponent={
                this.state.loading?
...
export default FindLeaguesScreen;
